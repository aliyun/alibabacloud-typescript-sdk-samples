...
export default class Client {
  ...
  static async schedPreloadJob(siteResponseBody: $ESA20240910.CreateSiteResponseBody, client: ESA20240910): Promise<$ESA20240910.CreateScheduledPreloadJobResponseBody> {
    console.info("Begin Call CreateScheduledPreloadJob to create resource");
    let createScheduledPreloadJobRequest = new $ESA20240910.CreateScheduledPreloadJobRequest({
      insertWay: "textBox",
      siteId: siteResponseBody.siteId,
      urlList: "http://test1.gositecdn.cn/test/test.txt",
      name: "testscheduledpreloadjob",
    });
    let createScheduledPreloadJobResponse = await client.createScheduledPreloadJob(createScheduledPreloadJobRequest);
    console.info("Call CreateScheduledPreloadJob success, response: ");
    console.info(Util.toJSONString(createScheduledPreloadJobResponse));
    return createScheduledPreloadJobResponse.body;
  }

  static async destroySchedPreloadJob(createScheduledPreloadJobResponseBody: $ESA20240910.CreateScheduledPreloadJobResponseBody, client: ESA20240910): Promise<void> {
    console.info("Begin Call DeleteScheduledPreloadJob to destroy resource");
    let deleteScheduledPreloadJobRequest = new $ESA20240910.DeleteScheduledPreloadJobRequest({
      id: createScheduledPreloadJobResponseBody.id,
    });
    let deleteScheduledPreloadJobResponse = await client.deleteScheduledPreloadJob(deleteScheduledPreloadJobRequest);
    console.info("Call DeleteScheduledPreloadJob success, response: ");
    console.info(Util.toJSONString(deleteScheduledPreloadJobResponse));
  }
  ...
  static async main(args: string[]): Promise<void> {
    ...
    // destroy resource
    await Client.destroySchedPreloadJob(schedPreloadJobRespBody, esa20240910Client);
  }
